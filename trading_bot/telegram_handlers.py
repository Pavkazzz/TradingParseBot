...
class MessageHandler:
    ...
    @bot.command(r'/start')
    async def start(self, chat: Chat, match):
        self.manager.start(chat.message["from"]["id"])
        await self.key(chat, match)

    @bot.command(r'/stop')
    async def stop(self, chat: Chat, match):
        self.manager.stop(chat.message["from"]["id"])
        await self.key(chat, match)

    @bot.command(r'^About$')
    @bot.command(r'/about')
    async def about(self, chat: Chat, _):
    ...
    @bot.command(r'/key')
    async def key(self, chat: Chat, _):
    ...
    async def smartlab(self, chat: Chat, _):
        sl = SmartLab()
    ...
    async def settings(self, chat: Chat, _):
        self.state = self.IDLE

        if self.manager.settings(chat.message["from"]["id"]).alenka:
    ...
    async def print_settings(self, chat: Chat, _):
        chat_id = chat.message["from"]["id"]
        current_settings = self.manager.settings(chat_id)
        msg = ""
    ...
    async def subscribe_alenka(self, chat: Chat, match):
        text, _ = await self.manager.new_command(chat.message["from"]["id"], Manager.ADD_ALENKA)
        await chat.send_text(text)
        await self.settings(chat, match)

    @bot.command(r'^Отписаться от ALЁNKA$')
    async def unsubscribe_alenka(self, chat: Chat, match):
        text, _ = await self.manager.new_command(chat.message["from"]["id"], Manager.REMOVE_ALENKA)
        await chat.send_text(text)
        await self.settings(chat, match)

    @bot.command(r'^MFD.ru тема')
    async def mfd_forum(self, chat: Chat, _):
    ...
    async def mfd_forum_add(self, chat: Chat, _):
        await chat.send_text("Введите имя темы или ссылку на тему или любое сообщение этой темы ",
                             reply_markup=keyboard_markup())
        self.state = self.MFD_THREAD_ADD

    @bot.command(r'^Удалить mfd тему$')
    async def mfd_forum_remove(self, chat: Chat, _):
        chat_id = chat.message["from"]["id"]
        await chat.send_text("Выберете тему для удаления: ",
                             reply_markup=keyboard_markup(
                                 [data.name for data in self.manager.settings(chat_id).mfd_thread], n_col=1)
                             )
        self.state = self.MFD_THREAD_REMOVE

    @bot.command(r'^MFD.ru пользователи')
    async def mfd_user(self, chat: Chat, _):
        options = ["Добавить mfd пользователя", "Удалить mfd пользователя"]
        await chat.send_text("Выберете действие: ", reply_markup=keyboard_markup(options))

    @bot.command(r'^Добавить mfd пользователя$')
    async def mfd_user_add(self, chat: Chat, _):
        await chat.send_text("Введите имя темы или ссылку на пользователя.\nЕсли передумали, введите \"Отмена\" ",
                             reply_markup=keyboard_markup())
        self.state = self.MFD_USER_ADD

    @bot.command(r'^Удалить mfd пользователя$')
    async def mfd_user_remove(self, chat: Chat, _):
        chat_id = chat.message["from"]["id"]
        await chat.send_text("Выберете пользователя для удаления: ",
                             reply_markup=keyboard_markup(
                                 [data.name for data in self.manager.settings(chat_id).mfd_user], n_col=1)
                             )
        self.state = self.MFD_USER_REMOVE

    @bot.default
    async def received_information(self, chat: Chat, _):
        if self.state == self.IDLE:
            return
        if self.state == self.MFD_THREAD_ADD:
            await self.mfd_add_thread(chat)
        if self.state == self.MFD_THREAD_REMOVE:
            await self.mfd_remove_thread(chat)
        if self.state == self.MFD_USER_ADD:
            await self.mfd_add_user(chat)
        if self.state == self.MFD_USER_REMOVE:
            await self.mfd_remove_user(chat)

    async def mfd_remove_user(self, chat: Chat):
        text = str(chat.message['text'])
        cid = chat.message["from"]["id"]
        res = ""
        for data in self.manager.settings(cid).mfd_user:
            if data.name == text:
                res, _ = await self.manager.new_command(cid, Manager.REMOVE_MFD_USER, data)
        if res:
            await chat.send_text(res)
            await self.settings(chat)
        else:
            await chat.send_text("Данный пользователь не найден для удаления. Введите правильное имя")

    async def mfd_remove_thread(self, chat: Chat):
        text = str(chat.message['text'])
        cid = chat.message["from"]["id"]
        res = ""
        for data in self.manager.settings(cid).mfd_thread:
            if data.name == text:
                res, _ = await self.manager.new_command(cid, Manager.REMOVE_MFD_THREAD, data)
    ...
    async def mfd_add_user(self, chat: Chat):
        text = str(chat.message['text'])
        rating = -1
        if ': ' in text:
            try:
        ...
        if text.startswith('http'):
            answer = await self.manager.resolve_mfd_user_link(cid, text)
            if not answer:
            ...
            users, res = await self.manager.find_mfd_user(cid, text, rating)
            if len(users) == 1:
                await chat.send_text(res)
                await self.settings(chat)
            if len(users) > 1:
                await chat.send_text('Найдено несколько пользователей. Уточните запрос или введите новый. Имя: Рейтинг',
                                     reply_markup=keyboard_markup([f"{user[1]} : {user[3]}" for user in users],
                                                                  n_col=1))
            if len(users) == 0:
                await chat.send_text('Пользователь не найден. Введите новый запрос')

    async def mfd_add_thread(self, chat: Chat):
        text = str(chat.message['text'])
        cid = chat.message["from"]["id"]

        if text.startswith('http'):
            answer = await self.manager.resolve_mfd_thread_link(cid, text)
            ...
            await chat.send_chat_action('typing')
            titles, res = await self.manager.find_mfd_thread(cid, text)
            if len(titles) == 1:
                await chat.send_text(res)
                await self.settings(chat)
            if len(titles) > 1:
