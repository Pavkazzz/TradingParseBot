...
| |  
| |  есть) у меня она сложнее, смотрю ev/ebitda и p/e
| | конечно, но расчитываю форвард на год+ и смотрю на
| | менеджмент это для меня оч важно
| | 
|  
|  Но в EV/EBITDA и P/E присутствует капитализация, что не
| является объективные показателем. Если не брать её во
| внимание, ты на что больше смотришь, на выручку,
| операционную прибыль или чистую прибыль?

смотри, я уже писал, для меня важнее менджмент и их
работа, это немного более сложный анализ.  
Если брать отчеты, то я смотрю операционку больше, чем ЧП
ибо в ней много шлака, выручка зависит от компаний - в
металлах смотрю, в киви нет, например. Тут компании разные

"""
        source = MfdSource()
        res = source.pretty_text(text)
